...
int main(int argc, char* argv[]) {
	...
	std::shared_ptr<Data> XDataA(pCLapp, XData::genTestXData(pCLapp, ColsA, RowsA, 1, type_index(typeid(realType)), XData::CONSTANT));
	std::shared_ptr<Data> XDataB(pCLapp, XData::genTestXData(pCLapp, ColsB, RowsB, 1, type_index(typeid(realType)), XData::CONSTANT));
	...
	std::shared_ptr<Data> XDataC(pCLapp, new XData(pCLapp, pArraysDims, pDynDims, type_index(typeid(realType))));
	...
	*/
	...
	auto profileParameters = std::make_shared<ProcessCore::ProfileParameters> (true, numberOfIterations);
	auto pProcess = Process::create<ArrayMultProcess>(pCLapp, profileParameters);
	...
	auto launchParamsArrayMultProcess = make_shared<ArrayMultProcess::LaunchParameters>(XDataB->getHandle(), RowsA, ColsA, ColsB, blockSize);
