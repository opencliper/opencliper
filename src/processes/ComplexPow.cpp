...
#include <OpenCLIPER/processes/ComplexPow.hpp>
...
void ComplexPow::init() {

    kernel = getApp()->getKernel("complexPow");

}

void ComplexPow::launch() {
 ...
 //       cl::NDRange globalWorkSize = cl::NDRange(nDArrayTotalSize);
...
//        queue.enqueueNDRangeKernel(kernel, cl::NullRange, globalWorkSize, localSizes, NULL, &event);
	kernelsExecEventList.push_back(event);

	stopProfiling();
	if(pProfileParameters->enable)
	    getKernelGroupExecutionTimes(kernelsExecEventList, "OpenCLIPER::ComplexPow::launch kernel", "OpenCLIPER::ComplexPow::launch group of kernels");

    }
    catch(cl::Error& err) {
    	BTTHROW(CLError(err), "ComplexPow::launch");
    }
}
